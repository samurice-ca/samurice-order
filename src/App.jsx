...
const PRODUCTS = [
  {
    sku: "ID21",                 // ← Odoo側の default_code と合わせる（重要）
    name: "Kinmemai White 5kg"   // ← 表示用
  }
];
...
export default function App() {
    ...
    const init = async () => {
      try {
        if (!LIFF_ID || LIFF_ID === "YOUR_LIFF_ID") {
          throw new Error("LIFF_ID が設定されていません。LINE Developers の LIFF ID を App.jsx に貼り付けてください。");
        }
        ...
        if (window.liff) {
          ...
          throw new Error("LIFF SDK (sdk.js) が読み込まれていません。index.html を確認してください。");
        }
      } catch (err) {
  ...
  if (complete) return (
    <div className="flex flex-col items-center justify-center min-h-screen p-10 text-center bg-white">
      ...
      <p className="text-slate-500 mt-2 text-sm">Make に送信しました（Odoo登録はMake側で実行）。</p>
    </div>
  );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 pb-32">
      {error && (
        <div className="bg-red-500 text-white p-3 flex items-center gap-2 text-xs font-bold italic">
          <AlertCircle size={14} />
          <span>ERROR: {error}</span>
        </div>
      )}

      <header className="bg-white p-6 border-b flex justify-between items-end">
        <div>
          <h1 className="font-black text-2xl tracking-tighter text-slate-900 italic leading-none">SAMURICE</h1>
          <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">Store: {storeId}</p>
        </div>
      ...
      <main className="p-4 space-y-4">
        <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-xl relative overflow-hidden">
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">Authenticated Customer</p>
        ...
        <div className="space-y-3">
          {PRODUCTS.map(p => (
            <div key={p.sku} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold">SKU: {p.sku}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 p-1 rounded-xl border">
                ...
                <span className="w-6 text-center font-bold text-slate-700">{cart[p.sku] || 0}</span>
